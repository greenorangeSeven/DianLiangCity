...
- (void)layoutSubviews
{
    ...
    CGFloat menuOptionButtonWidth = 44;
    if (self.tag == 100)
    {
        menuOptionButtonWidth = 55;
    }
    self.moreOptionsButton.frame = CGRectMake(width - menuOptionButtonWidth - menuOptionButtonWidth, 0., menuOptionButtonWidth, height);
...
- (UIButton *)moreOptionsButton
{
    if (!_moreOptionsButton) {
        ...
        if (self.tag == 100)
        {
            [_moreOptionsButton setTitle:@"支付" forState:UIControlStateNormal];
            _moreOptionsButton.backgroundColor = UIColorRGB(24, 210, 142);
        }
        else
        {
            [_moreOptionsButton setTitle:@"设为\n默认" forState:UIControlStateNormal];
            [_moreOptionsButton.titleLabel setLineBreakMode:NSLineBreakByCharWrapping];
            _moreOptionsButton.backgroundColor = UIColorRGB(170, 170, 170);
        }
        [_moreOptionsButton.titleLabel setFont:[UIFont systemFontOfSize:13]];
...
- (UIButton *)deleteButton
{
    if (self.editable) {
        if (!_deleteButton) {
            ...
            if (self.tag == 100)
            {
                [_deleteButton setTitle:@"忽略" forState:UIControlStateNormal];
                _deleteButton.backgroundColor = UIColorRGB(39, 96, 173);
                
            }
            else
            {
                [_deleteButton setTitle:@"解除\n绑定" forState:UIControlStateNormal];
                [_deleteButton.titleLabel setLineBreakMode:NSLineBreakByCharWrapping];
                _deleteButton.backgroundColor = UIColorRGB(230, 0, 9);
            }
