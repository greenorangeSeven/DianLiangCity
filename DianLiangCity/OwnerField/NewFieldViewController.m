...
@interface NewFieldViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIAlertViewDelegate,UIActionSheetDelegate, UIPickerViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate,VPImageCropperDelegate>
{
...
@implementation NewFieldViewController
...
- (void)submitBBS
{
    ...
    {
        [Tool showCustomHUD:@"请输入标题" andView:self.view andImage:nil andAfterDelay:1.2f];
        return;
    }
    ...
    if([[UserModel Instance] isNetworkRunning])
    {
        ...
        NSString *url = [NSString stringWithFormat:@"%@%@", api_base_url, api_add_bbs_subject];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:url]];
        [request setUseCookiePersistence:[[UserModel Instance] isLogin]];
        [request setPostValue:api_key forKey:@"APPKey"];
        [request setPostValue:titleStr forKey:@"subject"];
        [request setPostValue:contentStr forKey:@"content"];
        [request setPostValue:[NSString stringWithFormat:@"%i",self.currentChannel.id] forKey:@"channel_id"];
...
- (void)requestSubmit:(ASIHTTPRequest *)request
{
    ...
    if(status.intValue == 1)
    {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"noticReload" object:nil];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else
    {
