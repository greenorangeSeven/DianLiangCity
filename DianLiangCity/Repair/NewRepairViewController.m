...
@interface NewRepairViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIAlertViewDelegate,UIActionSheetDelegate, UIPickerViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate,VPImageCropperDelegate>
{
...
{
    [super viewDidLoad];
    self.navigationItem.rightBarButtonItems = [BackButton rightButton:self action:@selector(submitRepair) title:@"提交"];
    self.navigationItem.leftBarButtonItems = [BackButton leftButton:self action:@selector(backAction:) image:@"back_bg"];
    self.collectionView.dataSource = self;
    self.collectionView.delegate = self;
    [self.collectionView registerClass:[RepairImgCell class] forCellWithReuseIdentifier:@"RepairImgCell"];
    ...
    self.peopleTextField.text = userInfo.name;
    self.phoneTextField.text = userInfo.tel;
    [self reSizeCollectionView];
}

- (void)submitRepair
{
    NSString *contentStr = self.contextTextField.text;
    NSString *peopleStr = self.peopleTextField.text;
    NSString *phoneStr = self.phoneTextField.text;
    NSString *noteStr = self.noteTextFiled.text;
    if(contentStr.length <= 0)
    {
        [Tool showCustomHUD:@"请输入报修内容" andView:self.view andImage:nil andAfterDelay:1.2f];
        return;
    }
    if(peopleStr <= 0)
    {
        [Tool showCustomHUD:@"请输入报修联系人" andView:self.view andImage:nil andAfterDelay:1.2f];
        return;
    }
    ...
    if(noteStr.length <= 0)
    {
        [Tool showCustomHUD:@"请输入备注" andView:self.view andImage:nil andAfterDelay:1.2f];
        return;
    }
    ...
    {
        UserInfo *userInfo = [[UserModel Instance] getUserInfo];
        NSString *url = [NSString stringWithFormat:@"%@%@", api_base_url, api_add_baoxiu];
        ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:[NSURL URLWithString:url]];
        [request setUseCookiePersistence:[[UserModel Instance] isLogin]];
        [request setPostValue:api_key forKey:@"APPKey"];
        
        [request setPostValue:phoneStr forKey:@"contact_phone"];
        [request setPostValue:peopleStr forKey:@"contact"];
        [request setPostValue:contentStr forKey:@"summary"];
        [request setPostValue:noteStr forKey:@"remark"];
...
- (void)requestFailed:(ASIHTTPRequest *)request
{
    ...
    [Tool showCustomHUD:@"提交报修失败" andView:self.view andImage:nil andAfterDelay:1.2];
}
...
- (void)requestSubmit:(ASIHTTPRequest *)request
{
    ...
    {
        [Tool showCustomHUD:@"提交成功" andView:self.view andImage:nil andAfterDelay:1.2];
        [self.navigationController popViewControllerAnimated:YES];
    }
    else
    {
        [Tool showCustomHUD:@"报修失败,请重试" andView:self.view andImage:nil andAfterDelay:1.2];
    }
}
...
- (void)reSizeCollectionView
{
    ...
    float x = self.collectionView.frame.origin.x;
    float y = self.collectionView.frame.origin.y;
    ...
    self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, self.collectionView.frame.origin.y + self.collectionView.frame.size.height + 50);
}
